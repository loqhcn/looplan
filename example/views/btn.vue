<template>
    <div class="m-layout">
        ...
        <div>
            <div class="tip">使用lp-component加载Text组件</div>
            <lp-component is="looplan-example@Text">123123</lp-component>
        </div>

        <!-- <div style="margin-top: 16px;">
            <LpPaginate v-model:status="pageStatus" :showQuickJumper="true" />
        </div> -->
    </div>
</template>
...
const pageStatus = ref<PaginatePageStatus>({
  page: 1,
  limit: 10,
  total: 123,
  lastPage: Math.max(1, Math.ceil(123 / 10)),
  hasMore: 1 < Math.max(1, Math.ceil(123 / 10)),
})
</script>
<script lang="ts">
export default {
